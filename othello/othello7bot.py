...
def midgame_search(board, token, alpha, beta, depth):
    ...
    if not moves:
        ...
        if not opponent_moves:
            return brdEval(board, token)
        return -midgame_search(board, opponent, -beta, -alpha, depth - 1)
...
def playGame(myTkn):
    ...
    brd, tkn, enemy = '.'*27 + 'ox......xo' + '.'*27, 'x', 'o'
    xscript = []
    while True:
        myMoves = get_possible_moves(brd, tkn)
        enemyMoves = get_possible_moves(brd, enemy)
        if not myMoves and not enemyMoves:
            break
        if not myMoves:
            xscript.append(-1)
            tkn, enemy = enemy, tkn
    ...
    enemyScore = brd.count(enemy)
    print(f"Game took {duration:.2f} seconds. Result: {myScore}-{enemyScore}")
    return myScore, enemyScore, xscript
...
def runTournament(numGames):
    ...
    for gameNum in range(1, numGames + 1):
        start = time.time()
        myTkn = "xo"[gameNum % 2]
        score, enemyScore, _ = playGame(myTkn)
        end = time.time()
        totalScore += score
        print(f"Game {gameNum}: {'X' if myTkn == 'x' else 'O'} Score: {score} vs enemy score {enemyScore} in {end-start} seconds")
   
    avgScore = totalScore / (numGames*64) * 100
    winRate = (totalScore + numGames) / (2 * numGames)
